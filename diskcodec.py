...
def _check(data, descriptor):
    if (type(data) is not bytes or type(descriptor) is not dict
            or set(descriptor) != {'size', 'sha256'} or type(descriptor['size']) is not int
...
def dump_disk_state(disk, mode='auto', embed_limit=64*1024*1024):
    if mode not in ('auto', 'embed', 'reference') or type(embed_limit) is not int or embed_limit < 0:
    ...
    if type(disk) is str:
        ...
        if mode == 'auto':
            ...
            mode = 'embed'
        return ({'format': 'pypc.disk', 'version': 1, 'kind': 'file', 'mode': mode,
                 'image': _descriptor(data)}, {'image': data} if mode == 'embed' else {})
    if type(disk) is not HostDirectoryFAT16:
        raise ValueError('unsupported disk type')
    if mode == 'reference':
        raise ValueError('host mounts require embedding their live image and sync state')
    if set(vars(disk)) != {'directory', 'identity', '_image', '_host_paths', '_synced_files'}:
    ...
    paths = [{'key': [part.hex() for part in key],
              'path': _relative(path.relative_to(disk.directory).as_posix())}
             for key, path in disk._host_paths.items()]
    synced = [{'key': [part.hex() for part in key], 'size': size, 'chain': list(chain)}
              for key, (size, chain) in disk._synced_files.items()]
    manifest = {'format': 'pypc.disk', 'version': 1, 'kind': 'host', 'mode': 'embed',
                'image': _descriptor(buffers['image']), 'entries': entries,
                'paths': paths, 'synced': synced}
...
def validate_disk_state(manifest, buffers, reference=None):
    ...
    if (type(manifest) is not dict or manifest.get('format') != 'pypc.disk'
    ...
    if set(manifest) != common | ({'entries', 'paths', 'synced'} if host else set()):
        raise ValueError('invalid disk fields')
    if manifest['kind'] not in ('file', 'host') or manifest['mode'] not in ('embed', 'reference'):
    ...
    if manifest['mode'] == 'reference':
        if reference is None or buffers:
            raise ValueError('reference image required, without embedded buffers')
        image = Path(reference).read_bytes()
    else:
        image = buffers.get('image')
    _check(image, manifest['image'])
    names, expected_buffers = {}, {'image'} if manifest['mode'] == 'embed' else set()
    if host:
        ...
        for entry in manifest['entries']:
            ...
            keys = {'path', 'kind'} | ({'blob', 'data'} if entry['kind'] == 'file' else set())
            if set(entry) != keys:
        ...
        for name in names:
            for parent in PurePosixPath(name).parents:
                if str(parent) != '.' and names.get(str(parent)) != 'directory':
...
def restore_disk_state(manifest, buffers, destination, reference=None):
    ...
    for entry in sorted(manifest['entries'], key=lambda item: (item['path'].count('/'), item['path'])):
    ...
    disk._host_paths = {_key(item['key']): disk.directory/item['path'] for item in manifest['paths']}
    disk._synced_files = {_key(item['key']): (item['size'], tuple(item['chain'])) for item in manifest['synced']}
    return disk
