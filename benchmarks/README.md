...
## Live Pyro II through the JSON-RPC debugger
...
```sh
python benchmarks/pyro_rpc_ab.py --video vga --host-dir /absolute/path/to/scratch-game
```

Boot DOS (Enter acknowledges the GLaBIOS FDC warning), select `D:`, and run
`PYRO22`. On the first-run instruction page press End. Select a scenario with
Enter and let the animated demo begin. Keyboard input can be sent using the
normal `input.keyboard` JSON-RPC method at localhost:2301 or a VNC client.
...
```sh
python benchmarks/benchmark_rpc.py --pairs 10 --guest-ms 250 > pyro-live.json
```
