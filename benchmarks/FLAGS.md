...
All final-state comparisons passed. The full local suite passed 90 tests plus
184 subtests. CI reruns tests on Python 3.12/3.13 and uploads raw benchmark JSON.
Timing has no CI pass threshold: shared runners are noisy.

**These are synthetic instruction workloads, not a claim of a 1.5x Pyro
end-to-end speedup.** For live workloads, launch `python benchmarks/flags_rpc.py
...
Never mount the original private game's `bin` directory: the guest writes its
settings and records. Live sequential A/B samples advance the game and are not
